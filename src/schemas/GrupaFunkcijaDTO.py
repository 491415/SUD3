...
T = TypeVar("T", bound="GrupaFunkcijaDTO")

class GrupaFunkcijaDTO(BaseModel):
    """
    ...
    def as_dict(cls, dto_list: List[T]) -> List[Dict[str, Any]]:
        """
        Konverzija liste GrupaFunkcijaDTO u listu dictonarya koristeći Pydantic model_dump
        za serijalizaciju svakog objekta.

        Args:
            dto_list (List[T]): Lista GrupaFunkcijaDTO objekata za konverziju.
