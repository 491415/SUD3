...
T = TypeVar("T", bound="UpisVrstaUpisaDTO")

class UpisVrstaUpisaDTO(BaseModel):
    """
    ...
    upis_id: int = Field(..., ge=1, description="ID upisa.")
    vrsta_upisa_id: int = Field(..., ge=1, description="ID vrste upisa.")
    ...
    def as_dict(cls, dto_list: List[T]) -> List[Dict[str, Any]]:
        """
        Konverzija liste UpisVrstaUpisaDTO u listu dictonarya koristeći Pydantic model_dump
        za serijalizaciju svakog objekta.

        Args:
            dto_list (List[T]): Lista UpisVrstaUpisaDTO objekata za konverziju.
