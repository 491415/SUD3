...
T = TypeVar("T", bound="GrupaVrstaUpisaDTO")


class GrupaVrstaUpisaDTO(BaseModel):
    """
    ...
    id: int = Field(..., ge=1, description="ID grupe (primarni ključ).")
    ...
    def as_dict(cls, dto_list: List[T]) -> List[Dict[str, Any]]:
        """
        Konverzija liste GrupaVrstaUpisaDTO u listu dictonarya koristeći Pydantic model_dump
        za serijalizaciju svakog objekta.

        Args:
            dto_list (List[T]): Lista GrupaVrstaUpisaDTO objekata za konverziju.
