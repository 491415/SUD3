...
T = TypeVar("T", bound="SubjektiDTO")


class SubjektiDTO(BaseModel):
    """
    ...
    sud_id_nadlezan: int = Field(..., ge=1, le=999_999_999_999, description="ID nadležnog suda.")
    sud_id_sluzba: int = Field(..., ge=1, le=999_999_999_999, description="ID stalne službe ili nadležnog suda ako nema stalne službe.")
    postupak: int = Field(..., ge=0, le=9, description="Postupak u kojem se subjekt nalazi (stečaj, likvidacija, ...).")
    oib: Optional[int] = Field(None, ge=0, le=99_999_999_999, description="Osobni identifikacijski broj subjekta.")
    mb: Optional[int] = Field(None, ge=0, le=99_999_999, description="Matični broj poslovnog subjekta (dodjeljuje DZS).")
    ...
    mbs_brisanog_subjekta: Optional[int] = Field(None, ge=1, le=999_999_999, description="Brisani subjekt iza kojeg ovaj subjekt nastaje (samo za stečaju i likvidacijsku masu).")
    glavna_djelatnost: Optional[int] = Field(None, ge=0, le=99_999, description="Šifra glavne djelatnosti prema NKD šifrarniku.")
    glavna_podruznica_rbr: Optional[int] = Field(None, ge=1, le=9_999, description="Redni broj trenutno glavne podružnice inozemnog osnivača.")
    ...
    sud_id_brisanja: Optional[int] = Field(None, ge=1, le=999_999_999_999, description="ID suda koji je brisao subjekt iz registra.")
    ...
    def as_dict(cls, dto_list: List[T]) -> List[Dict[str, Any]]:
        """
        Konverzija liste SubjektiDTO u listu dictonarya koristeći Pydantic model_dump
        za serijalizaciju svakog objekta.

        Args:
            dto_list (List[T]): Lista SubjektiDTO objekata za konverziju.
