...
class SudDTO(ReferencaPodatkaDTO):
    """
    ...
    id: int = Field(..., ge=0, description="Primary key ID")
    ...
    sifra_opcine: int = Field(..., ge=1, le=99999, description="Šifra općine.")
    naziv_opcine: str = Field(..., min_length=1, max_length=128, description="Naziv općine.")
    sifra_naselja: int = Field(..., ge=1, le=9999999999, description="Šifra naselja.")
    naziv_naselja: str = Field(..., min_length=1, max_length=128, description="Naziv naselja.")
    sud_id_nadlezan: Optional[int] = Field(None, ge=1, description="ID nadležnog suda (samo za stalne službe).")
